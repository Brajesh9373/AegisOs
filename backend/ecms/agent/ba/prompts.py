...
COVERAGE_DIMENSIONS = [
    ...
    ("frontend", "Frontend: UI surfaces, web/mobile targets, framework, accessibility and UX constraints"),
    ("backend", "Backend: services, APIs, runtime/framework, auth boundaries and business logic ownership"),
    ("database", "Database: data stores, schema ownership, volumes, migration/seeding and retention"),
...
def build_reply_prompt(
    ...
    critical_open = [
        d for d in open_dims
        if d.get("key") in _CRITICAL_DIMENSIONS
    ]
    ...
    if ready:
        readiness = (
            ...
            if not all_sufficient else
            "READINESS: Every dimension is sufficient. Offer to generate the "
    ...
    categories = ", ".join(
        f"{key} ({label})" for key, label in CLARIFICATION_CATEGORIES.items()
    )
...
ASSESS_TOOL = {
    ...
    "function": {
        ...
        "description": (
            "Report requirement-coverage status for every dimension of the "
...
FINALIZE_TOOL = {
    ...
    "function": {
        ...
        "parameters": {
                ...
                "phases": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            ...
                            "startDate": {"type": "string", "description": "Phase start date in YYYY-MM-DD format"},
                            "endDate": {"type": "string", "description": "Phase end date in YYYY-MM-DD format"},
                            "meetingFrequency": {"type": "string", "enum": ["daily", "every2days", "weekly", "biweekly", "monthly"], "description": "How often to schedule meetings during this phase"},
                        },
                        "required": ["name", "duration", "description", "startDate", "endDate", "meetingFrequency"],
                    },
                },
            ...
            "required": [
                "projectName", "objective", "functionalReqs", "techStack",
                "skills", "connectors", "risks", "phases",
                "governance", "guardrails", "infrastructure",
            ],
        },
    },
}
                ...
                "properties": {
                    "agents": {
                        ...
                        "items": {
                            "type": "object",
                            "properties": {
                                "key": {"type": "string", "description": "Stable slug, unique within the team; reports_to targets it."},
                                "name": {"type": "string"},
                                "designation": {"type": "string", "description": "Human-readable job title shown in the workspace."},
                                "role": {"type": "string", "description": "Free-text job title you choose for this project (e.g. 'Delivery Manager', 'ML Platform Lead', 'Mobile Engineer')."},
                                "department": {"type": "string", "description": "Free-text discipline/pod (e.g. 'backend', 'data', 'security')."},
                                "reports_to": {"type": ["string", "null"], "description": "The key of this agent's manager; null only for the single root."},
                                "goal": {"type": "string", "description": "One-sentence objective."},
                                "instructions": {"type": "string", "description": "Concrete operating instructions for this agent."},
                                "skills": {"type": "array", "items": {"type": "string"}},
                                "model": model_schema,
                                "tools": {"type": "array", "items": tool_enum},
                            },
                            "required": [
                                "key", "name", "designation", "role", "department",
                                "reports_to", "goal", "instructions", "skills", "model", "tools",
                            ],
                        },
                    },
                    "revised_phases": {
                        ...
                        "items": {
                            ...
                            "properties": {
                                "name": {"type": "string"},
                                "startDate": {"type": "string", "description": "Revised start date in YYYY-MM-DD format"},
                                "endDate": {"type": "string", "description": "Revised end date in YYYY-MM-DD format"},
                                "meetingFrequency": {"type": "string", "enum": ["daily", "every2days", "weekly", "biweekly", "monthly"]},
                            },
                    ...
                    "org_mappings": {
                        ...
                        "items": {
                            "type": "object",
                            "properties": {
                                "project_agent_key": {"type": "string", "description": "The key of the project agent (must match a TeamAgent.key)."},
                                "org_member_id": {"type": "string", "description": "The id of the permanent org member from the roster."},
                                "responsibility": {"type": "string", "enum": ["primary_owner", "monitor", "approver"], "description": "How the org member relates to this agent."},
                            },
