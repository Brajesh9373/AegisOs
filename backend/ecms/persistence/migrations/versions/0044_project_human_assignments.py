...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "responsibility", sa.String(length=64), nullable=False, server_default="workspace_owner"
        ),
        ...
        sa.ForeignKeyConstraint(
            ["organization_member_id"], ["organization_members.id"], ondelete="RESTRICT"
        ),
        sa.ForeignKeyConstraint(
    ...
    op.create_index(
        "ix_project_human_assignments_project_id", "project_human_assignments", ["project_id"]
    )
    op.create_index(
        "ix_project_human_assignments_member_id",
        "project_human_assignments",
        ["organization_member_id"],
    )
    op.create_index(
        "ix_project_human_assignments_position_id", "project_human_assignments", ["position_id"]
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_project_human_assignments_position_id", table_name="project_human_assignments"
    )
