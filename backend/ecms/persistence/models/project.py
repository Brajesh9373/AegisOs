...
class Project(Base):
    ...
    connectors: Mapped[list["ProjectConnector"]] = relationship(
        "ProjectConnector", back_populates="project", cascade="all, delete-orphan",
    )
...
class ProjectConnector(Base):
    ...
    project_id: Mapped[str] = mapped_column(String(128), ForeignKey("projects.id", ondelete="CASCADE"), nullable=False, index=True)
